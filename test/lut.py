...
class Coeffs(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = UNet(n_channels=6)

        
        
    def forward(self, x1, x2):
        inputs = F.interpolate(torch.cat((x1, x2), dim=1), size=[128, 128],   mode='bilinear', align_corners=True)
...
class StudentNet(nn.Module):
    ...
    def __init__(self):
        ...
        nn.Conv2d(in_channels=3*4*4, out_channels=128, kernel_size=3, stride=1, padding=1),
        nn.ReLU(),
    ...
    def forward(self, low, high):
        coeffs = self.coeffs(low,  high)
        guide = self.guide(0.5 *low + 0.5 * high)
        ...
        result = trilinear_(self.d3(slice_coeffs), 0.5 *low + 0.5 * high)
        return result
